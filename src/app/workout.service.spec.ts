...
describe('WorkoutService', () => {
  ...
  it('should add a new workout type for an existing user', () => {
    ...
    const storedWorkouts = service.getAllItems('workouts');
    console.log(
      'Stored Workouts After Adding New Workout Type:',
      storedWorkouts
    );
    expect(storedWorkouts.length).toBe(1);
    expect(storedWorkouts[0].workouts.length).toBe(2);
    expect(
      storedWorkouts[0].workouts.find(
        (w: { type: string; minutes: number }) => w.type === 'Running'
      )?.minutes
    ).toBe(30);
    expect(
      storedWorkouts[0].workouts.find(
        (w: { type: string; minutes: number }) => w.type === 'Swimming'
      )?.minutes
    ).toBe(45);
  });
